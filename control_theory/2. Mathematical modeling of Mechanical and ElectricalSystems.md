...
(1)$$ J\ddot{\theta} + b \dot{\theta} = K i $$

(2)$$ L \frac{di}{dt} + Ri = V - K\dot{\theta} $$
...
(3)$$ s(Js + b)\Theta(s) = KI(s) $$

(4)$$ (Ls + R)I(s) = V(s) - Ks\Theta(s) $$
...
(5)$$ P(s) = \frac{\dot{\Theta}(s)}{V(s)} = \frac{K}{(Js + b)(Ls + R) + K^2} \quad [\frac{rad/sec}{V}] $$
