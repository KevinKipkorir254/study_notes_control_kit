...
covariance matrices.
$$\theta = p \sum_{r=1}^n [p^{r-1}]^{-1} \theta_r$$ 
$$\mathbf{P} = \left[ \sum_{r=1}^n [p^{r-1}]^{-1} \right]^{-1}$$ (15)

$$\mathbf{P}$$ will then also be the covariance matrix of the resulting estimate $$\theta$$.
...
[^1]: Consider discussing pre-filtering in this context.
