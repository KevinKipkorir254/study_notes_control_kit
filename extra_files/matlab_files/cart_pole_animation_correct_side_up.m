...
function animate_cart_pole(t, s, l)
    ...
    for i = 1:length(t)
        ...
        % Adjust the angle so that it points downward when theta = pi
        theta_adjusted = theta - pi;

        % Calculate pendulum position
        pendulum_x = x + pole_length * sin(theta_adjusted);
        pendulum_y = -pole_length * cos(theta_adjusted);  % Corrected for orientation
